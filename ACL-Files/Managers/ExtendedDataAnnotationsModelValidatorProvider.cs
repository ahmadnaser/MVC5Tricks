...
using AhmadNaserCollector.Attributes;
using AhmadNaserCollector.Managers;

namespace AhmadNaserCollector.Managers
{
    /// <summary>
                ...
                if (ValidationManager.Validators.ContainsKey(metadata.ContainerType.Name))
                {
                    ...
                    if (validator.ContainsKey(metadata.PropertyName))
                    {
                        ...
                        if (property.Visible)
                        {
                            ...
                            {
                                System.ComponentModel.DataAnnotations.CompareAttribute compare = new System.ComponentModel.DataAnnotations.CompareAttribute(property.Compare) { ErrorMessage = property.ErrorMessage };
